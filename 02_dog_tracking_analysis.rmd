...
mm1<-lmer(log(distance.sum)~owner_presence:stranger_presence+owner_presence+
            ...
            (1+owner_presence+stranger_presence|subject), weights=1/prop_tracked_data, data=hc.data, REML=FALSE)
...
mm2<-lmer(log.distance.sum~owner_presence+stranger_presence+
            z.trial+sex+z.age+
            (1+owner_presence+stranger_presence|subject), data=hc.data, weights=1/prop_tracked_data, REML=FALSE)
...
```{r}
library(car)
xx=lm(log(distance.sum)~owner_presence+stranger_presence+cBARQ_separation_problems+z.cBARQ_nonsocial_fear+z.trial+z.age+sex, data=hc.data)
vif(xx)
```
...
tail.tip.data<-xdata%>%
  filter(keypoint=="tail_base" | keypoint=="tail_tip")%>%
  select(subject, keypoint, distance.sum, owner_presence, stranger_presence, condition, trial_number, sex, age, cBARQ_sep_attach )%>%
...
tail.tip.data$z.cBARQ_sep_attach<-scale(tail.tip.data$cBARQ_sep_attach, center = TRUE, scale=TRUE)
...
mm1_area=glmmTMB(area_covered~owner_presence*stranger_presence+
            z.trial+z.age+sex+
            (1|subject)+ (0+owner_presence|subject)+(0+stranger_presence|subject),
...
drop1(mm1_area, test="Chisq")#convergence problems
```

```{r}
library(car)
xx=lm(area_covered~owner_presence+stranger_presence+z.trial+z.age+sex + z.cBARQ_sep_attach, data=hc.data)
vif(xx)
```
...
mm2_area=glmmTMB(area_covered~owner_presence+stranger_presence+            z.trial+z.age+sex+
            (1|subject)+ (0+owner_presence|subject)+(0+stranger_presence|subject),
           #(0+z.trial|subject), #pruned due to convergence issues
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)
...
```{r}
drop1(mm2_area, test="Chisq",  control=contr)#convergence problems
```
...
model_table_area <- bind_cols(as.data.frame(summary(mm2_area)$coefficients$cond)) %>% 
  select(Estimate, SE = `Std. Error`, `z value`, p = `Pr(>|z|)`) %>%
  mutate(across(.cols = c(p), ~ format(round(.x, 3), nsmall=3))) %>% 
  mutate(across(.cols = c(Estimate:`z value`), ~ format(round(.x, 2), nsmall=2))) %>% 
  #mutate(across(Chi2:p, ~replace_na(.x, "")))%>%
mutate(p=replace(p, p==0, "<0.001"))
...
mm1_duration_owner=glmmTMB(prop_duration_owner.scaled ~owner_presence*stranger_presence+ z.trial+z.age+sex+
            (1|subject)+ (0+owner_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence
...
mm2_duration_owner=glmmTMB(prop_duration_owner.scaled ~owner_presence+stranger_presence+ z.trial+z.age+sex+
            (1|subject)+ (0+owner_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence
anova(mm2_duration_owner, mm1_duration_owner_null, test="Chisq")
summary(mm2_duration_owner)
```


```{r}
drop1(mm2_duration_owner, test="Chisq",  control=contr)#convergence problems
```
### output table
```{r}
...
write.csv(model_duration_owner, file = "saves/duration_owner_mm2_output_table.csv")
```
...
mm1_duration_stranger=glmmTMB(prop_duration_stranger.scaled~owner_presence*stranger_presence+ z.trial+
            (1|subject)+ (0+owner_presence|subject)+ (0+stranger_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence

mm1_duration_stranger_null=glmmTMB(prop_duration_stranger.scaled~ z.trial+
            (1|subject)+ (0+owner_presence|subject)+ (0+stranger_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)
...
mm2_duration_stranger=glmmTMB(prop_duration_stranger.scaled ~owner_presence+stranger_presence+ z.trial+sex+
            (1|subject)+ (0+owner_presence|subject)+ (0+stranger_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence

mm2_duration_stranger_null=glmmTMB(prop_duration_stranger.scaled ~ z.trial+sex+
            (1|subject)+ (0+owner_presence|subject)+ (0+stranger_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence
...
```{r}
drop1(mm2_duration_stranger, test="Chisq",  control=contr)#convergence problems
```
...
mm1_duration_door=glmmTMB(prop_duration_door.scaled~owner_presence*stranger_presence+ z.trial+z.age + 
            (1|subject)+ (0+owner_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence

mm1_duration_door_null=glmmTMB(prop_duration_door.scaled~ z.age + 
            (1|subject)+ (0+owner_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)
...
mm2_duration_door=glmmTMB(prop_duration_door.scaled ~owner_presence+stranger_presence+z.trial+
            (1|subject)+ (0+owner_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence

mm2_duration_door_null=glmmTMB(prop_duration_door.scaled ~ 1+
            (1|subject)+ (0+owner_presence|subject),
             weights=prop_tracked_data_inv,
             data=hc.data, family=beta_family, control=contr)#pruned due non-convergence
...
```{r}
drop1(mm2_duration_door, test="Chisq",  control=contr)#convergence problems
```
...
corrplot(corr[13:16,],method="color",   p.mat = p.mat[13:16,], sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.9, insig = 'label_sig', pch.col = 'grey20', tl.col = 'black')
...
levels(hc.data$condition2)
vis.angle.owner.plot2 <- ggplot(data = hc.data, aes(y = mean_vis_angle_owner_scaled, x = condition2, fill = condition2)) +
geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
geom_point(aes(y = mean_vis_angle_owner_scaled, color = condition2), position = position_jitter(width = .15), size = 1, alpha = 0.8) +
geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.5) +
expand_limits(x = 5.25) +
guides(fill = FALSE) +
guides(color = FALSE) +
...
raincloud_theme+
xlab("") + 
ylab("Vis. angle to owner chair (scaled)")+
  ylim(0,0.8)
...
```{r}
library(cowplot)
pg2<-plot_grid(distance.plot2, object.distance.plot2, area.plot2, tail.plot2, ncol=2,  labels=c("A", "B", "C", "D"))

ggsave(pg2, filename = "graphics/dog_exploration_pg3.png", width=12, height=12, scale=0.85)
```

```{r}

library(cowplot)
pg3<-plot_grid(owner.duration.plot , stranger.duration.plot , door.duration.plot , nrow=3,  labels=c("B", "C", "D"), label_size = 20)
...
ggsave(pg4, filename = "graphics/dog_exploration_duration_pg4.png", width=13, height=9, scale=1.4)
```
...
pg6<-plot_grid(p.example, pg5, labels = c("A", ""), nrow=2, rel_heights = c(1,1.3))
